...
빅데이터 기술과 기계학습과 인공지능이 결합하여 그동안 난제로 여겨졌던 다양한 문제에 대해서 
해답을 제시함으로써 신형 은총알(silver bullet)로 부각되고 있지만, 
언제나 그렇듯이 남은 과제가 무엇인지 살펴보고 `tidyverse`를 통해 해결하려고 하는 것이 무엇인지 간략히 짚어보자.
...
테크기업 혹은 전통산업에 데이터 과학을 접목시키고자 신입 혹은 경력직 사원에 대한 채용이 늘고 있다.
채용과정에서는 딥러닝, 인공지능, 기계학습에 대한 내용에 상당부분 할애되는 경우가 다반사다.
하지만, 신규 직원에 대한 조기 퇴사율이 높은 이유를 설명하는데, 다음에 나온 데이터 과학에 대한 불편한 진실이 도움이 될 듯 싶다.
...
이런 질문에 영국의 통계학자 에드워드 심슨이 정리한 역설(Simpson paradox)은 데이터 과학 시대를 살아가는 우리에게 시사하는 바가 크다고 할 수 있다.
...
데이터 과학자가 주로 다루는 데이터는 실험 데이터 보다는 관측 데이터가 일반적이다.
...
전산학과나 데이터 과학을 공부할 수 있는 이공계로 진학하는게 더 유리한 세상이 되었다. 

전통 제조업을 대표하는 자동차회사, 서비스업의 총아 금융회사에서 소프트웨어와 데이터 과학 전문인력을 많이 필요로 하는 이유를 살펴보자.
...
이러한 도전에 대해 설명가능한 기계학습/인공지능 기술과 결합하여 현존하는 데이터 과학에 대한 인간의 지적 자산이 응축된 `tidyverse`가 영역을 넓혀 큰 역할을 담당하게 될 것이다.
