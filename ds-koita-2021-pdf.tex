...
사고를 갖춘 과학 연구자를 요구하고 있다. 데이터 집약 과학은 기존
직사각형 형태의 정형 데이터를 넘어 텍스트로 대표되는 비정형 데이터를
자원으로 삼아 연구성과를 한단계 높일 수 있다. 텍스트를 자원으로 가치를
창출하는 자연어 처리가 각광을 받고 있으며 이를 위해 OCR을 도입하여 더
많은 연구개발 성과물을 디지털화시켜 연구개발 빅데이터를 구축하고 있다.
기계독해와 자연어 이해기술을 통해 과학 연구개발을 한단계 도약시킬 수
있을 뿐만 아니라 재현가능한 데이터 과학 논문 저작기법을 도입하여
버전제어와 형상관리를 통해 투명성을 높이고 특정 소프트웨어와 데이터에
종속되지 않도록 협업할 수 있는 기반을 마련하여 과학기술 연구자들의
생산성도 높이고 있다.
\\
\keywords{
        자동화 \and
...
\citep{rushkoff2010program} 라는 표현을 통해서 일자리에 대한 위험을
...
\end{figure}
\footnotetext{\href{https://www.youtube.com/watch?v=ZaEihnQwR9s}{김광두
